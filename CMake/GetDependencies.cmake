...
function(get_all_dependencies target output_list)
        ...
        elseif (NOT in_special_block)
            if (TARGET ${dep})  # Exclude non-targets like m (= libm)
                ...
                if (NOT _is_system OR NOT NO_SYSTEM)
                    ...
                    message(STATUS "QtAppBase: get_all_dependencies ignoring ${target} -> ${dep} (system)")
                endif ()
            else ()
                message(STATUS "QtAppBase: get_all_dependencies ignoring ${target} -> ${dep} (not a target)")
            endif ()
        else ()
            message(STATUS "QtAppBase: get_all_dependencies ignoring ${target} -> ${dep} (added externally)")
        endif ()
    endforeach ()
    ...
    if (ARG_NO_STATIC)
        foreach (dep IN LISTS all_deps)
            ...
            if (dep_type STREQUAL "STATIC_LIBRARY")
                message(STATUS "QtAppBase: get_all_dependencies pruning ${target} -> ${dep} (static)")
    ...
    foreach (dep IN LISTS all_deps)
        get_target_property(dep_type ${dep} TYPE)
        if (dep_type STREQUAL "INTERFACE_LIBRARY" OR dep_type STREQUAL "EXECUTABLE" OR dep_type STREQUAL "OBJECT_LIBRARY")
            message(STATUS "QtAppBase: get_all_dependencies pruning ${target} -> ${dep} (not a library)")
            list(REMOVE_ITEM all_deps ${dep})
        endif ()
    endforeach ()
